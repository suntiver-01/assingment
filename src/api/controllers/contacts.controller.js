...
const {
  validateCreateGroup,
} = require("../validations/oncreateGroup.validation");
const { validateUser } = require("../validations/user.validation");
const {
  validateContact,
} = require("../validations/oncreateContact.validation");
const { validateEditGroup } = require("../validations/updateGroup.validation");
const {
  validateUpdateContact,
} = require("../validations/updateContact.validation");

const methods = {
  async onLogin(req, res, next) {
    const { error } = validateUser(req.body);
    if (error)
      return res.error({ message: error.details[0].message, statusCode: 400 });
  ...
  async onUpdateUser(req, res, next) {
    ...
    if (!userId)
      return res.error({ message: "Invalid userID!", statusCode: 400 });
  ...
  async onUpdateGroup(req, res, next) {
    ...
    if (!groupId)
      return res.error({ message: "Invalid groupId!", statusCode: 400 });
  ...
  async onCreateContact(req, res, next) {
    ...
    let body = {
      contact: { ...req.body, contactId: uuid(), image: req?.file?.filename },
    };

    let contact = await Group.findOneAndUpdate(
    ...
    if (!contact)
      return res.error({
        message: "Invalid user or  groupId!",
        statusCode: 400,
      });
  ...
  async onUpdateContact(req, res, next) {
    ...
    let contactId = await Group.find({
      "contact.contactId": req.params.contactId,
    });
    if (contactId.length == 0)
      return res.error({ message: "Invalid contactId!", statusCode: 400 });
  ...
  async onDeleteContact(req, res, next) {
    ...
    await Group.updateOne(
      { "contact.contactId": req.params.contactId },
      { $pull: { contact: { contactId: req.params.contactId } } }
    );
